...
- All automation is **tick-based**. The Extract Port checks for items to extract every few ticks. On fast servers, lower `extractPortInterval`. On busy servers, increase it to reduce load.
- Automation blocks only work when placed **adjacent** to the Storage Core. If you break and move the core, automation blocks need to be repositioned.
- `minSyncInterval` in the config (default: 2 ticks) controls how often the storage GUI syncs to clients. Setting this to 0 can make automation appear more responsive but increases network traffic.
