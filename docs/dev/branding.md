...
The mod has been successfully rebranded from **EZStorage 2** to **Steve's Simple Storage (S3)**. All package structures,
mod IDs, and asset directories have been updated.
...
The name is inspired by AWS S3 (Simple Storage Service), creating a fun parallel between cloud storage and Minecraft
storage systems. The personal connection (developer's name is also Steve) adds authenticity to the Minecraft Steve
reference.
...
"Steve's Simple Storage (S3) is a Minecraft mod that provides massive, scalable storage through an intuitive multiblock
system. Inspired by AWS S3, it makes storing millions of items simple and accessible."
